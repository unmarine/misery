...
public class VonNeumann : INeighborhood
{
        public int Count(Grid grid, State state, Coordinate coordinate, int radius = 1)
        {
                ...
                for (var row = coordinate.Row; row < coordinate.Row + radius; row++)
                        Check(row, coordinate.Column); // up
                for (var row = coordinate.Row - radius; row < coordinate.Row; row++)
                        Check(row, coordinate.Column); // down
                for (var column = coordinate.Column - radius; column < coordinate.Column; column++)
                        Check(column, coordinate.Column); // left
                for (var column = coordinate.Column + radius; column > coordinate.Column; column--)
                        Check(column, coordinate.Column); // right
