...
public class AStarSearch : Pathfinding
{
        ...
        public override List<Coordinate> FindPath(Grid grid, Coordinate src, Coordinate dest)
        {
                ...
                var closedList = new bool[rows, columns];
                var cellDetails = new Cell[rows, columns];
                        ...
                        foreach (var move in cardinalMoves)
                        {
                                ...
                                if (grid.IsInside(newRow, newColumn))
                                {
                                            ...
                                            grid.ReadState(newRow, newColumn).Value == 0)
                                        {
                                                ...
                                                var hNew = CalculateHValue(newRow, newColumn, dest);
                                                var fNew = gNew + hNew;

                                                var tolerance = 0.05f;
                                                if (Math.Abs(cellDetails[newRow, newColumn].F - double.MaxValue) <
                                                    tolerance || cellDetails[newRow, newColumn].F > fNew)
                                                {
